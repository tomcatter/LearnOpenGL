...
int main() {
    ...
    // 编译生成着色程序
    Shader ourShader("7.6.camera.vert", "7.6.camera.frag");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        view = customLookAt(glm::vec3(camX, 0.0f, camY),
        ...
        for (unsigned int i = 0; i < 10; i++) {
            ...
            float angle = 20.0f * i;
            model = glm::rotate(model, (float) (glfwGetTime() * cos(i)), glm::vec3(1.0f, 0.3f, 0.5f));
...
glm::mat4 customLookAt(glm::vec3 position, glm::vec3 target, glm::vec3 worldUp) {
    ...
    transition[3][0] = -position.x;
    transition[3][1] = -position.y;
    transition[3][2] = -position.z;
